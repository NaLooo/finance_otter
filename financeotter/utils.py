...
def get_summary(ticker: str):
    ...
    if not soup:
        return
    div = soup.find(name=DIV, id='quote-summary')
    result = div.find_all(TD)
    index = []
    data = []
    flag = True
    for row in result:
        if flag:
            index.append(row.span.string.strip())
        else:
            if row.span:
                data.append(row.span.string.strip())
            else:
                data.append(row.string.strip())
        flag = not flag
